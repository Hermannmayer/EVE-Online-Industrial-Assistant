...
SKILL_CATEGORIES = [
    ...
    # 精炼产率只由这两项决定（`core/eve_formulas.calc_refining_yield`：
    # 基础 ×(1+3%×提炼学概论) ×(1+2%×提炼效率理论)）。查询页「精炼产物」面板按人物
    # 读这里填的等级算产率，所以必须能在人物设置里填。
    #
    # 矿石专精（`Veldspar Processing` 这类**按矿种**的技能）这次不进表：公式里的
    # `ore_skill_level` 目前全链路都是 0，而 SDE 没有「矿种 → 处理技术」的现成映射，
    # 要接得先建那张映射表。加进来只会让人以为填了有效。
    (
        "精炼",
        "recycle",
        [
