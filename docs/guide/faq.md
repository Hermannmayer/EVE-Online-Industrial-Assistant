...
1. 看**主工具栏**左侧的「价格时效」（`价格: N 分钟前`，绿/黄/红三档）判断数据新不新
2. 点**主工具栏右侧的「更新价格」**手动刷新（这是唯一的手动更新入口）
3. 通过 **系统 → 数据初始化** 重新拉取
...
现行口径（`domain/formulas.py`，2026-09-16 起统一为「整批取整」）：

```
整批用量 = ceil(SDE 数量 × 作业次数 × (100 - ME) / 100 × 结构减免)
```

`SDE 数量` 取 `blueprint_materials.quantity`（已是 ME 0 时的实际用量，含基础浪费）。
取整**只做一次、作用于整个作业** —— 先逐轮取整再乘作业次数会系统性多要货
（基础量 22、ME10、2510 次作业：49,698 对 50,200）。每轮基础量 ≤ 1 的材料不吃 ME。
「废品率 / Waste Factor」是 wasteFactor 时代的字段，SDE 已移除，现行公式不再使用。
