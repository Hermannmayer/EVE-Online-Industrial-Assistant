...
**新增用例前先读 CLAUDE.md 的「测试边界」、克制条款第 4 条与「测试判定表」 —— 测试有硬预算，不按覆盖率补用例。**
**先过判定表确认这次改动到底要不要写测试**：外观断言 / 标准库断言 / 仅非 None / mock 调用次数都在禁止档；
判定表外的用例可用 `python scripts/test_audit.py` 自查。
