...
| 态 | 区域名 | 代码定位 | 内容 |
|---|---|---|---|
| 空闲态 | 仪表盘 | `#queryDashboard` | 三栏：产线详情（`components/OccupancyPanel.qml`，**每人物一块、块内制造/科研/反应三行**，行尾给「待下线 N」，**该列恒定预留宽度**，条子分母取该线型各人物上限的最大值；工业页「人物占用情况」对话框渲染的是同一块面板，只差口径 —— 那边按「已规划」算）/ 资产折线图（`AssetChartPanel`，5 条线 + 右上角「刷新」`#assetRefreshButton`；涨跌基准取档位起点**之前**最近一条快照）/ 挂单列表（`OpenOrdersPanel`，**卖单 `#sellOrdersTable` 在上、买单 `#buyOrdersTable` 在下两张表** + `#readOrdersButton`；钱包余额 `#walletField`） |
