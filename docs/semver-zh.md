...
-
有相同主版本号、次版本号及修订号的两个先行版本号，其优先层级必须（MUST）通过由左到右的每个被句点分隔的标识符来比较，直到找到一个差异值后决定：
-
只有数字的标识符以数值高低比较。
-
...
（原文为 ABNF 语法定义，此处略。校验版本号直接用本文末尾 FAQ 中的官方正则表达式，效果等同。）
...
参见：[https://regex101.com/r/vkijKf/1/](https://regex101.com/r/vkijKf/1/)
```
