...
```python
def active_lines_by_category(plans: list[dict], *, statuses: tuple[str, ...]=RUNNING_STATUSES) -> dict[str, dict[str, int]]
```
...
```python
def char_line_usage(plans: list[dict], *, statuses: tuple[str, ...]=RUNNING_STATUSES) -> tuple[list[tuple[str, dict[str, tuple[int, int]]]], dict[str, int]]
```
