...
def _fetch(sql, rid: int, params=None):
    with get_container().db.connect('ref', 'mkt') as conn:
        ...
        for row in c.fetchall():
            ...
            r.append({
                "mg": mg, "id": tid, "z": zh or "", "e": en or "",
                "v": vol or 0, "bp": bp, "sp": sp, "ap": ap,
                "bv": bv or 0, "sv": sv or 0,
            })
        return r
    ...
    def __init__(self, tid, parent=None):
        ...
        with get_container().db.connect('ref', 'mkt', 'bp') as conn:
            ...
            lay.addWidget(
                QLabel(
                    f"制造材料: {nm}",
                    styleSheet=f"color:{theme.PRIMARY};font-size:13px;font-weight:bold;"
                )
            )
        ...
        lay.addWidget(
            QLabel(
                f"总成本: {total:,.2f} ISK",
                styleSheet=f"color:{theme.ACCENT_GREEN};font-size:12px;font-weight:bold;"
            )
        )
        b = QPushButton("关闭")
        b.clicked.connect(self.accept)
    ...
    def run(self):
        ...
        with get_container().db.connect('ref', 'mkt', 'bp') as conn:
            ...
            for row in c.fetchall():
                mg,tid,zh,en,vol,bp,sp,bv,sv=row
                ap=((bp or 0)+(sp or 0))/2 if bp and sp else (bp or sp)
                r.append({"mg": mg, "id": tid, "z": zh or "", "e": en or "",
                         "v": vol or 0, "bp": bp, "sp": sp, "ap": ap,
                         "bv": bv or 0, "sv": sv or 0})
            self.done.emit(r)
...
class AllItemsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self._pr.setStyleSheet(f"QProgressBar{{background:{theme.BG_SURFACE};border:none;border-radius:1px;}}QProgressBar::chunk{{background:{theme.PRIMARY};border-radius:1px;}}")
        ...
        def _do_add_plan():
            ...
            try:
                ...
                conn.execute(
                    ...
                    "VALUES (?,?,?,?,?,?,?,?,?,?,'pending',?,?,?,?,?,?)",
                    (tid, _ctx_name, data["runs"], data["parallels"], data["me"], data["te"],
                     self._mfg["hub"], self._mfg["hub"], data["fac"], data["char"],
                     score.get("profit_per_run", 0), score.get("margin_pct", 0),
                     score.get("score", 0), iskph, mat_cost,
    ...
    def _ds(self,r,is_mfg):
        if is_mfg:
            ...
            mat_lines="\n".join(
                f"  {m['name']} x{m['qty']:,} @ {m['unit_price']:,.2f} = {m['subtotal']:,.0f}"
                for m in mats
            )
            return (
                f"每批利润核算\n"
                ...
                f"经纪人(改单): {b.get('broker_rate',0)*rev/100*(1-b.get('relist_discount',50)/100):.0f} ISK"
