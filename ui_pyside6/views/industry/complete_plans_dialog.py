...
def complete_plans(plans: list[dict], hangar_id: int, *, parent=None, ask_outcome: bool = True) -> dict:
    """把一批 ready 计划下线到指定机库。
    ...
    Returns: {"completed": int, "deposited": int, "failed": [产品名...], "skipped": [产品名...]}
    """
    completed = 0
    deposited = 0
    failed: list[str] = []
    ...
    for plan in plans:
        ...
        if ask_outcome and _is_pending_invention(plan):
            ...
            if actual is None:  # 用户取消
                ...
                continue
        res = plan_execution.complete_plan(plan, actual_output_runs=actual)
        if res.get("ok"):
            completed += 1
