...
| 功能 | 说明 |
|------|------|
| 🔍 **前缀候选** | 输入中/英文名或 Type ID，实时列出**全部**前缀匹配（名字按**开头**匹配，条数不限、自带滚动） |
| 🎯 **候选即详情** | 点候选那一条直接出详情（**没有结果表格**，也不再有「搜索」这一步） |
| 📖 **详情面板** | 四块：5 个贸易中心价格 / 实时 Top5 买单卖单 / 精炼产物 / 制造所需的材料 |
| ⚗️ **按人物算精炼产率** | 精炼面板可选**人物 + 数量 + 站点**，产率按该人物的提炼技能算（技能在「人物设置 → 精炼」填） |
| 🕐 **搜索历史** | 点一下空搜索框显示最近 20 条记录 |
| 📊 **价格走势图** | 历史价格趋势可视化（入口待接回，代码已在仓库内） |
| 📋 **批量导出** | 查询结果导出为 CSV/Excel（全物品浏览页） |
