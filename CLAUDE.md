...
**写在哪 —— 决定「要不要写」之后，先过这三条**

| 情形 | 做法 |
|---|---|
| 断言能并进已有用例 | **并进去，不新开**。要加的多半是既有用例的**同一个分支**（先例：`test_asset_summary_baseline_falls_back_to_earliest` 与 `test_asset_summary_rows_delta` 覆盖同一分支，本该合并成一条） |
| 骨架第二次出现 | 加进**参数化列表**或收敛成**共享实现**，不抄第二遍。先例：`tests/test_qml_dialogs.py` 的 `_LOADS_CASES`（44 个对话框一条用例）、`tests/qml_page_load.py`（5 个页面的「能加载 + 无告警」收敛成各一行 —— 收敛前是 5 份逐字复制的 18 行骨架） |
| 桥 / 控制器只是转发 | 默认 **0 条**。方法体是一行 `self._page.x()` 时，用例只是接线冒烟；整个文件最多留 1 条，不要一个方法一条 |

体检：`scripts/test_audit.py`（手动跑，不挂钩子）报出上述禁止项，只报告不改动。
它只按**单条断言**启发式匹配；「同名用例被抄了几份」这类**结构重复测不出来** ——
2026-09-20 试过三种口径，信噪比都不合格（同名口径 6 条里多为同名不同物；同文件同断言 177 条；
跨文件代码块口径 1 条且是误报），所以没装进去。那一类靠上面「写在哪」三条自己把关。
