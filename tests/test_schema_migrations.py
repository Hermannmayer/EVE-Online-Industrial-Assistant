...
def _create_user_v14(db_path):
    """构造 v14 的 production_plans 表（缺 v15 的 material_cost_snapshot 列）。

    直接用当前建表 DDL 剥掉 v15 新列 —— 不手抄一份 v14 全量列，避免将来加列时
    这里成为第二个需要同步的地方。剥离失败会当场断言，不会假通过。

    注：另一个 `_create_user_v12`（科研列那条线）由本文件后半段定义，用途不同，
    故本夹具改名为 v14 以免重名互相覆盖。
    """
    from services.repositories.plan_repository import PlanRepository

    line = "        material_cost_snapshot TEXT DEFAULT '',\n"
    assert line in PlanRepository.SCHEMA, "建表 DDL 结构变了，请同步本夹具的剥离逻辑"
    conn = sqlite3.connect(str(db_path))
    conn.executescript(PlanRepository.SCHEMA.replace(line, ""))
    cols = {r[1] for r in conn.execute("PRAGMA table_info(production_plans)")}
    assert "material_cost_snapshot" not in cols, "夹具应模拟缺该列的 v14 库"
...
def test_user_v3_to_v4_adds_execution_columns(tmp_user_db):
    ...
    assert "material_cost_snapshot" in cols, "material_cost_snapshot 列应被 v14→v15 迁移添加"


def test_user_v14_to_v15_adds_material_cost_snapshot(tmp_user_db):
    """user v14→v15：新增启动成本快照列，且不动其它列。

    版本号说明：本迁移原登记为 v12→v13，与「科研作业列」相撞；合并时顺延到 v14→v15。
    """
    _create_user_v14(tmp_user_db)
...
def test_no_backup_when_already_current(tmp_user_db):
    """库已是最新版本 → 不触发备份"""
    conn = sqlite3.connect(str(tmp_user_db))
    # 用常量而不是写死数字：本用例的语义就是「已经最新」，写死会随每次迁移失效
    conn.execute(f"PRAGMA user_version = {sm.DB_SCHEMA_VERSIONS['user']}")
    conn.commit()
    conn.close()
...
# ────────────────────────────────────────────
#  user v16 → v17：asset_snapshots / open_orders
# ────────────────────────────────────────────


def test_user_v16_to_v17_creates_dashboard_tables(tmp_user_db):
    """user v16→v17：新增 asset_snapshots / open_orders 两张表（仅加表，不改既有列）。"""
    conn = sqlite3.connect(str(tmp_user_db))
...
# ────────────────────────────────────────────
#  user v17 → v18：line_value 列 / order_events 表
# ────────────────────────────────────────────


def test_user_v17_to_v18_adds_line_value_and_order_events(tmp_user_db):
    """user v17→v18：asset_snapshots 加 line_value 列 + 新增 order_events 台账表。"""
    conn = sqlite3.connect(str(tmp_user_db))
    conn.executescript(sm._USER_V17_TABLES_SQL)  # 先造出一个真正的 v17 库（有那两张表）
...
# ────────────────────────────────────────────
#  user v18 → v19：esi_tokens 表
# ────────────────────────────────────────────


def test_user_v18_to_v19_adds_esi_tokens(tmp_user_db):
    """user v18→v19：新增 esi_tokens 表（按角色绑定的 ESI 刷新令牌）。

    这张表存**不可重建**的用户数据 —— 刷新令牌丢了就得让用户重新走一次
    浏览器授权，所以它必须是真迁移，不能靠运行时 CREATE 兜底。
    """
    conn = sqlite3.connect(str(tmp_user_db))
...
def test_user_v19_to_v20_adds_order_owner_columns(tmp_user_db):
    """user v19→v20：`open_orders` 加归属列 `char_id` / `is_corp`。

    挂单有两个来源（游戏日志 / ESI）、一个账号可绑多个角色、个人单与军团单又是两份
    独立导出 —— 没有归属列时变动识别拿全表做差，会把别的组判成「已成交」，
    进而**错误增减钱包**（会算错钱，所以这条必须有测试钉住）。
    存量行补 0（归属未知）而不是编造一个角色。
    """
    from services.schema_migrations import _USER_V17_TABLES_SQL

    # 用真 DDL 剥掉这两行 —— 不手抄一份 v19 全量列，否则将来加列时这里要同步第二次
    legacy = "".join(
        line
        for line in _USER_V17_TABLES_SQL.splitlines(keepends=True)
        if not line.strip().startswith(("char_id ", "is_corp "))
    )
    assert "char_id" not in legacy and "is_corp" not in legacy, "建表 DDL 结构变了，请同步本用例的剥离逻辑"

    conn = sqlite3.connect(str(tmp_user_db))
    conn.executescript(legacy)
    conn.execute("INSERT INTO open_orders (order_id, is_buy, price) VALUES (7, 0, 100.0)")
...
#  幂等性 / 缺表跳过 —— 参数化收敛
#
#  此前每个版本各写 1 条 *_idempotent 与 1 条 *_skips_missing_table，结构完全
...
def _seed_v4(path):
    ...
    _create_user_v4(path)


def _seed_v5(path):
    from tests.conftest import _create_user_v5

    _create_user_v5(path)


def _seed_v8(path):
    _create_user_v8_missing_v2_columns(path)


def _seed_v12(path):
    _create_user_v12(path)


def _seed_v13_stamped(path):
    """v12 表结构 + user_version=13（版本先行，科研列缺失）→ 走 v13→v14 修复。"""
    _create_user_v12(path)
...
def _seed_v15_negative_runs(path):
    _create_user_v15_with_negative_runs(path)


def _seed_v16(path):
    """空库 + user_version=16 → 触发 v16→v17 建表迁移。"""
    conn = sqlite3.connect(str(path))
    conn.execute("PRAGMA user_version = 16")
    conn.commit()
    conn.close()
...
@pytest.mark.parametrize("seed", [_seed_v3, _seed_v4, _seed_v5])
def test_user_add_column_migrations_idempotent(tmp_user_db, seed):
    """重复运行 ensure_schema：不报错，且不重复加列。"""
    seed(tmp_user_db)
    ...
    tables = [r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")]
    cols = {
        t: [r[1] for r in conn.execute("PRAGMA table_info(t)")] for t in ("production_plans", "hangars") if t in tables
    }
    conn.close()
    # 加列迁移的共性：目标列在第二次运行后仍只有一份（未重复添加）
    for t, tcols in cols.items():
        assert len(tcols) == len(set(tcols)), f"{t} 出现重复列名"


@pytest.mark.parametrize("seed", [_seed_v8, _seed_v12, _seed_v13_stamped, _seed_v15_negative_runs, _seed_v16])
def test_user_data_migrations_idempotent(tmp_user_db, seed):
    """重复运行 ensure_schema（含回填/补列/原图归一数据迁移）→ 第二次无变更。"""
    seed(tmp_user_db)
...
@pytest.mark.parametrize(
    "pragma",
    ["PRAGMA user_version = 3", "PRAGMA user_version = 4", "PRAGMA user_version = 5", "PRAGMA user_version = 9"],
)
def test_user_migrations_skip_when_no_tables(tmp_user_db, pragma):
    """库为空（无表）时迁移跳过、不抛异常，且版本推进到最新。"""
    conn = sqlite3.connect(str(tmp_user_db))
    conn.execute(pragma)
    conn.commit()
    conn.close()
...
def test_user_v15_to_v16_skips_missing_table(tmp_user_db):
    """无 user_blueprints 表的库 → 跳过原图归一，但不阻断版本推进。"""
    conn = sqlite3.connect(str(tmp_user_db))
    conn.execute("PRAGMA user_version = 15")
    conn.commit()
    conn.close()
